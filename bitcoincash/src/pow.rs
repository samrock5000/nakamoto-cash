...
/// asada
pub const MAX_ATTAINABLE_MAINNET: Target = Target(U256(0xFFFF_u128 << (208 - 128), 0));
...
impl Target {
    ...
    pub fn is_met_by(&self, hash: BlockHash) -> bool {
        // use hashes::Hash;
        let hash = U256::from_le_bytes(hash.as_hash().into_32());
        hash <= self.0
    }
    ...
    pub fn difficulty(&self, network: Network) -> u128 {
        let max = match network {
            ...
            Network::Chipnet => Target::MAX_ATTAINABLE_TESTNET,
...
mod tests {
    ...
    //     let target = Target(U256::from_le_bytes(hash.as_hash.into_32()));
