...
use crate::internal_macros::{serde_string_impl, serde_struct_human_string_impl, write_err};
...
use super::token;
use super::token::unwrap_scriptpubkey;
use super::token::wrap_scriptpubkey;
...
pub struct TxOut {
    ...
    pub token: Option<token::OutputData>,
}

// This is used as a "null txout" in consensus signing code.
impl Default for TxOut {
    fn default() -> TxOut {
        TxOut { value: 0xffffffffffffffff, script_pubkey: Script::new(), token: None }
    }
}
...
impl<E> EncodeSigningDataResult<E> {
    ...
    /// # use bitcoincash::{Transaction, Sighash};
    /// # use bitcoin_hashes::{Hash, hex::FromHex};
    /// # let mut writer = Sighash::engine();
    /// # let input_index = 0;
    /// # let script_pubkey = bitcoincash::Script::new();
...
impl Transaction {
    ...
    pub fn ntxid(&self) -> sha256d::Hash {
        let cloned_tx = Transaction {
            ...
            input: self
                .input
                .iter()
                .map(|txin| TxIn { script_sig: Script::new(), ..*txin })
                .collect(),
    ...
    pub fn signature_hash(
        ...
        if self
            .encode_signing_data_to(&mut engine, input_index, script_pubkey, sighash_u32)
            .is_sighash_single_bug()
...
impl Encodable for TxOut {
    fn consensus_encode<W: io::Write + ?Sized>(&self, writer: &mut W) -> Result<usize, io::Error> {
        ...
        Ok(if self.token.is_some() {
            let script = wrap_scriptpubkey(self.script_pubkey.clone(), &self.token);
...
impl Decodable for TxOut {
    fn consensus_decode<R: io::Read + ?Sized>(reader: &mut R) -> Result<Self, encode::Error> {
        ...
        let (script_pubkey, token) = match unwrap_scriptpubkey(script_pubkey) {
            Ok((s, t)) => (s, t),
            Err(_e) => {
                return Err(encode::Error::ParseFailed("failed to parse tokendata in txout"))
            }
        };
        Ok(TxOut { value, script_pubkey, token })
    }
}
...
mod tests {
    ...
    #[test]
    fn test_serialize_tokentx() {
        // Transaction that sends 42 of token 94d03dfec9efb0c7238dd12dab01f64159d8c8fb7ca4acb74043bbfe603b6d9d to bchreg:qq5m8hpmtm5cmfq8mvsfxktwv89ne4dfxyq8mrdmxg
        let txhex = "";

        let tx: Transaction = deserialize(&Vec::from_hex(txhex).unwrap()).unwrap();
        assert_eq!(42, tx.output[0].token.as_ref().unwrap().amount);
        assert_eq!(
            "94d03dfec9efb0c7238dd12dab01f64159d8c8fb7ca4acb74043bbfe603b6d9d",
            tx.output[0].token.as_ref().unwrap().id.to_hex()
        );

        let serialized = serialize(&tx);
        assert_eq!(txhex, serialized.to_hex());
        assert_eq!(tx, deserialize(&serialized).unwrap());
    }
}
