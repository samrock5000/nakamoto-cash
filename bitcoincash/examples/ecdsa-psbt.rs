...
use bitcoincash::hashes::hex::{self, FromHex};
use bitcoincash::secp256k1::{Secp256k1, Signing, Verification};
use bitcoincash::util::address;
use bitcoincash::util::amount::ParseAmountError;
use bitcoincash::util::bip32::{
    self, ChildNumber, DerivationPath, ExtendedPrivKey, ExtendedPubKey, Fingerprint,
    IntoDerivationPath,
};
use bitcoincash::util::psbt::{self, Input, Psbt, PsbtSighashType};
use bitcoincash::{
...
impl WatchOnly {
    ...
    fn create_psbt<C: Verification>(&self, secp: &Secp256k1<C>) -> Result<Psbt> {
        ...
        let tx = Transaction {
            ...
            output: vec![
                TxOut { value: to_amount.to_sat(), script_pubkey: to_address.script_pubkey(), token: None },
                TxOut {
    ...
    fn finalize_psbt(&self, mut psbt: Psbt) -> Result<Psbt> {
        use bitcoincash::util::psbt::serialize::Serialize;
...
fn previous_output() -> TxOut {
    ...
    TxOut { value: amount.to_sat(), script_pubkey, token: None }
}
...
mod psbt_sign {
    ...
    use bitcoincash::psbt::{Input, Prevouts, Psbt, PsbtSighashType};
    use bitcoincash::util::sighash::{self, SighashCache};
    use bitcoincash::util::taproot::TapLeafHash;
    use bitcoincash::{
